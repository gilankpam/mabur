/* ported from waybeam_venc f956a52:include/star6e_ipu.h */
#ifndef STAR6E_IPU_H
#define STAR6E_IPU_H
 ...
 * pointer table + global instance).
 *
 * mabur edit: upstream compiled this only into detection builds; mabur has
 * no detector but DOES need star6e_ipu_scrub(), which is unconditional
 * bring-up (see its doc comment below and star6e_runtime_init), so the file
 * is always in the venc_core build.  Only star6e_ipu_fmt_name() — a
 * detector-side diagnostic with no caller here — was dropped.
 *
...
/* mabur edit: star6e_ipu_fmt_name() dropped — detector-only diagnostic,
 * no caller in mabur. */
