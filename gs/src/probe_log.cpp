...
ProbeLog::ProbeLog(LogWriter& w, const std::string& dir, int bpb)
    : w_(w), s_(w.open(dir, "probe.log",
                       "probelog 3 bpb=" + std::to_string(bpb))) {}

void ProbeLog::row(double t_ms, uint32_t seq, int mcs, int bw, uint16_t enh_fid,
  ...
  const int n = std::snprintf(
      b, sizeof(b), "%.0f %u %d %d %u %d %u %.1f %.1f %.1f %.1f %.3f", t_ms,
      seq, mcs, bw, enh_fid, blocks_ok, card_mask, snr0, snr1, evm0, evm1, first_ms);
  if (n > 0)
