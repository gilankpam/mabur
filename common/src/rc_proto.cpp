...
std::optional<Telem> parse_telem(const uint8_t* buf, size_t len) {
  ...
  t.idr_grants = get16(buf, 63);
  t.vanished_base = get16(buf, 65);
  t.vanished_enh = get16(buf, 67);
  t.self_idr_refused = get16(buf, 69);
  return t;
}
