...
// idx is SIGNED and RELATIVE to the chip's own efuse TXAGC anchor (spec
// 2026-09-13-relative-walls), not an absolute TXAGC index -- it lives in
// the chip's 7-bit signed diff-field range, [-64, 63].
...
struct CalFrameInfo {
  ...
  int8_t idx = 0;     // index RELATIVE to the chip's anchor, [-64, 63]
...
inline std::vector<uint8_t> build_cal_payload(uint8_t rate, int8_t idx,
                                              uint8_t phase, uint16_t seq) {
  std::vector<uint8_t> p(kCalPayloadLen);
  std::memcpy(p.data(), kCalMagic, 4);
  p[4] = rate;
  p[5] = static_cast<uint8_t>(idx);
  p[6] = phase;
  p[7] = static_cast<uint8_t>(seq & 0xFF);
  p[8] = static_cast<uint8_t>((seq >> 8) & 0xFF);
  const uint8_t fill = static_cast<uint8_t>(0x5A ^ static_cast<uint8_t>(idx));
...
inline bool parse_cal_payload(const uint8_t* p, size_t len, CalFrameInfo* out) {
  ...
  const int8_t idx = static_cast<int8_t>(p[5]);
  const uint8_t phase = p[6];
  if (rate > 7) return false;
  if (idx < -64 || idx > 63) return false;
  if (phase != kPhaseCoarse && phase != kPhaseFine && phase != kPhaseVerify)
    return false;
  const uint8_t fill = static_cast<uint8_t>(0x5A ^ p[5]);
