...
struct Rig {
  ...
  bool step(const OsdComposeIn& in) {
    if (!comp.wants(back, in)) return false;
...
TEST(the_msp_stale_blank_reaches_both_buffers) {
  const mabur::MspScreen scr = full_screen('X');
  const GsSnapshot sn = snap_cards(3);
  const GsPlayerState ps = player(60.0);
  Rig r;
  ...
  CHECK(rounds == 2);  // exactly one composition per buffer, then it settles
  CHECK(r.px(0, kProbeX, kProbeY) == 0u);
  CHECK(r.px(1, kProbeX, kProbeY) == 0u);
}
...
TEST(an_alternating_value_draws_nothing_yet_must_still_be_published) {
  ...
  for (int i = 0; i < 8; ++i) {
    ...
    REQUIRE(r.comp.wants(r.back, in));
...
TEST(a_committed_buffer_always_matches_a_from_scratch_composition) {
  ...
  for (int i = 0; i < 200; ++i) {
    ...
    const OsdComposeIn in =
        make_in(&screens[cur_screen], fresh, stale, &sn, player(fps), gsd);
    if (!r.step(in)) continue;
    r.commit();
    const auto ref = reference(in, true, true, seen);
    const int wrong = diff_px(r.buf[r.front].px, ref);
    if (wrong != 0) {
      size_t k = 0;
      while (r.buf[r.front].px[k] == ref[k]) ++k;
      std::printf("  seed %u iteration %d: %d px wrong, first at (%d,%d) %08X != %08X\n",
                  seed, i, wrong, (int)(k % W), (int)(k / W), r.buf[r.front].px[k], ref[k]);
    }
    CHECK(wrong == 0);
    ++checked;
  }
