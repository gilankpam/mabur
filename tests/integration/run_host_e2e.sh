...
# stream; RC_VERSION 7, 2026-09-10, added T_CAL_CMD/T_CAL_RESULT and widened
# Telem; RC_VERSION 8, 2026-09-13, made calibration indices relative to the
# chip anchor and shrank Telem -- neither touches the RCF layout, only the
# version byte moves here. encode_profile and the CRC are unversioned.
body = struct.pack("<HBBBIHBBBB", rc_proto.RC_MAGIC, 8, rc_proto.T_RCF, 0,
...
sys.path.insert(0, os.path.abspath(os.path.join("..", "devourer", "tools", "precoder")))
import rc_proto
# Same head as the RCF above, but probe_profile is 0x06 (HT mcs6, 20 MHz)
