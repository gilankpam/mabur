...
TEST(disabled_passes_through) {
  ...
  DmaFrame repl;
  bool did = false;
  CHECK(reg.offer(frame(0), kT0, &repl, &did));
  CHECK(!did);
  CHECK(!reg.holding());
}
...
TEST(first_frame_seeds_floor_and_holds_exactly_d) {
  ...
  DmaFrame repl, out;
  bool did = false;
  CHECK(!reg.offer(frame(0), kT0, &repl, &did));  // held
  CHECK(!did);
  CHECK(reg.holding());
...
TEST(steady_cadence_releases_on_pts_grid_not_arrival) {
  ...
  CHECK(!reg.offer(frame(kFrame), a1, &repl, &did));
...
TEST(late_frame_presents_immediately) {
  ...
  CHECK(reg.offer(frame(kFrame), a1, &repl, &did));  // present now
  CHECK(!did);
  CHECK(!reg.holding());
...
TEST(floor_leaks_upward_to_track_drift) {
  ...
  for (int i = 1; i <= 3000; ++i) {
    ...
    if (!reg.offer(frame(static_cast<uint32_t>(pts)), arrival, &repl, &did))
...
TEST(newer_frame_replaces_held_mailbox_style) {
  ...
  CHECK(!reg.offer(frame(kFrame), kT0 + 1'000, &repl, &did));
  CHECK(did);
  CHECK(repl.opaque == frame(0).opaque);  // caller must free this one
...
TEST(pts_wrap_is_invisible) {
  ...
  CHECK(!reg.offer(frame(p0), kT0, &repl, &did));
  CHECK(reg.release_due(kT0 + kD, &out));
  const uint32_t p1 = p0 + static_cast<uint32_t>(kFrame);  // wrapped
  CHECK(!reg.offer(frame(p1), kT0 + kFrame, &repl, &did));
  CHECK(!reg.release_due(kT0 + kFrame + kD - 1'000, &out));
  CHECK(reg.release_due(kT0 + kFrame + kD + 16, &out));
}
...
TEST(pts_discontinuity_resets_and_passes_through) {
  ...
  CHECK(reg.offer(frame(5'000'000), kT0 + 2'000, &repl, &did));
  CHECK(did);
  CHECK(repl.opaque == frame(0).opaque);
  CHECK(!reg.holding());
