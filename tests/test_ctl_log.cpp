...
TEST(ctl_log_writes_header_and_records) {
  ...
  REQUIRE(log.ok());
  log.sample(1000, 2, 0.05, 31.5, 0.0, 0.10, 0.0, -24.5, 0.0);
  ...
  CHECK(text.rfind("ctllog 2 ladder=0/100,2/50 down_util=0.35 up_util=0.15\n", 0) == 0);
  CHECK(text.find("\nS 1000 2 0.0500 31.5 0.0000 0.1000 0.0000 -24.5 0.0000\n") != std::string::npos);
...
TEST(ctl_log_bad_dir_is_nonfatal) {
  ...
  CHECK(!log.ok());
  log.sample(0, 0, 0, 0, 0, 0, 0, 0, 0);   // must not crash
}
...
TEST(ctl_log_v2_sample_carries_resid_cur) {
  ...
  REQUIRE(log.ok());
  log.sample(1234, 3, 0.0123, 31.5, 0.0456, 0.0, 0.0, -21.0, 0.0011);
  std::string text = read_all(log.path());
  CHECK(text.rfind("ctllog 2 ladder=5/25 down_util=0.60 up_util=0.15\n", 0) == 0);
  CHECK(text.find("\nS 1234 3 0.0123 31.5 0.0456 0.0000 0.0000 -21.0 0.0011\n") != std::string::npos);
}
