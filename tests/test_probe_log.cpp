...
TEST(probe_log_header_row_and_name) {
  ...
  log.row(1234, 99, 6, 40, 17, 3, 0b11, 30.5, std::nan(""), -24.0, -22.5, 1130.4375);
  w.flush_now();
  std::string text = read_all(log.path());
  CHECK(text.rfind("probelog 3 bpb=4\n", 0) == 0);
  ...
  CHECK(text.find("\n1234 99 6 40 17 3 3 30.5 nan -24.0 -22.5 1130.438\n") != std::string::npos);
}
TEST(probe_log_bad_dir_is_nonfatal) {
  maburgs::LogWriter w;
  maburgs::ProbeLog log(w, "/nonexistent-dir-xyz", 4);
  CHECK(!log.ok());
  log.row(0, 0, 0, 20, 0, 0, 0, 0, 0, 0, 0, 0);
}
MTEST_MAIN
