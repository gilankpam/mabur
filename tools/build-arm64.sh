...
# --- 3. Configure + build. Only the maburgs target: devourer's example
...
cmake --build build-arm64 -j"$(nproc)" --target maburgs
...
"$(readlink -f toolchain/file)/bin/file" out/arm64/maburgs
